...
public class GenerateStudent {
    ...
    public static String[] grades = {"AA", "BB","BA", "CB", "BB", "AA","CB", "BA", "BB","CC","BA", "DC","BB", "DD","AA","DD", "DD"};// 7
    private static String[] firstNames   = { "Adam", "Alex", "Aaron", "Ben", "Carl", "Dan", "David", "Edward", "Fred", "Frank", "George", "Hal", "Hank", "Ike", "John", "Jack", "Joe", "Larry", "Monte", "Matthew", "Mark", "Nathan", "Otto", "Paul", "Peter", "Roger", "Roger", "Steve", "Thomas", "Tim", "Ty", "Victor", "Walter"};
    ...
    public static void setStudents(ArrayList<Student> students) {
        GenerateStudent.students = students;
    }

    private static Transcript trans = new Transcript();// just for calculating gpa before creating a random transcript.

    GenerateStudent(int numOfStudentsPerYear, int semester, int maxFailed){
        generate_students(numOfStudentsPerYear, semester, maxFailed);
    }

    public static void generate_students(int numOfStudentsPerYear, int semester, int maxFailed){
        int year = 2016;
    ...
    public static Transcript generate_transcript(int rand_semester, int maxFailed){
        ...
        for (int i = 1; i < rand_semester; i++){
           ...
           // ArrayList<Integer> totalTakenCredit = new ArrayList<Integer>();
            if (i == 1){
                active_courses = Registration.getAvailableCoursesForSimulation(i, 0.0,
                        globalPassedCourses, globalFailedCourses, globalNotTakenCourses);
            }
            else{
                active_courses = Registration.getAvailableCoursesForSimulation(i, trans.calculateTermGPA(passedCo, failedCo),
                        globalPassedCourses, globalFailedCourses, globalNotTakenCourses);
            }

            Registration.replaceElectives(active_courses);
            ...
            for (int j = 0; j < failedCount; j++){
                ...
                if (i > 2){ // check after second semester.
                   boolean condition = Registration.checkPrerequisites(fail, globalPassedCourses);
                    if (condition == false){
            ...
            for (int j = 0; j < limit; j++){
                ...
                // CHECK PREREQUISITES
                if (i > 2){ // check after second semester.
                    boolean condition = Registration.checkPrerequisites(pass, globalPassedCourses);
                    if (condition == false){
